...
OCDSpec2Context(AiSpec) {
  ...
  Describe(@"Impossible Ai", ^{
    ...
    Describe(@"getMove", ^{
        ...
        It(@"makes a smart move", ^{
            [board replaceSlot:0 withMark:@"X"];
            [ExpectInt([Ai getMove:board withMark:@"O"]) toBe:4];
        });
    });
    ...
    Describe(@"minimax", ^{
        ...
        It(@"state of original board should be same", ^{
            [board replaceSlot:0 withMark:@"O"];
            [board replaceSlot:1 withMark:@"X"];
            [board replaceSlot:2 withMark:@"O"];
            [Ai minimax:board withMark:@"O" maxPlayer:NO alpha:-50 beta:50 color:1 depth:0];
            
            [ExpectObj([board.slots objectAtIndex:3]) toBeEqualTo:@"4"];
        });
